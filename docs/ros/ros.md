...
<code>/Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*/cmd_vel</code>
...
<code>/Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*/desired_pose</code>
...
<code>/Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*/sensors/*&lt;camera_name&gt;*/desired_pose</code>
([geometry_msgs/PoseStamped](http://docs.ros.org/en/api/geometry_msgs/html/msg/PoseStamped.html))
...
### Published topics
<code>/Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*/actual_pose</code>
([geometry_msgs/PoseStamped](http://docs.ros.org/en/api/geometry_msgs/html/msg/PoseStamped.html))
<ul><li style="list-style-type: none;">Robot's current pose.</li></ul>

<code>/Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*/sensors/*&lt;camera_name&gt;*/*&lt;image_type&gt;*/image</code>
...
<code>/Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*/sensors/*&lt;camera_name&gt;*/*&lt;image_type&gt;*/camera_info</code>
...
<code>/Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*/sensors/*&lt;lidar_name&gt;*/lidar</code>
...
<code>/Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*/sensors/*&lt;radar_name&gt;*/radar_detections</code>
...
<code>/Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*/sensors/*&lt;radar_name&gt;*/radar_tracks</code>
...
<code>Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*</code>
<ul><li style="list-style-type: none;">Robot's base transform frame from the <code>map</code> parent frame ID.</li></ul>

<code>Sim/*&lt;scene_name&gt;*/robots/*&lt;robot_name&gt;*/sensors/*&lt;camera_name&gt;*/*&lt;image_type&gt;*</code>
<ul><li style="list-style-type: none;">The camera image's transform frame from the <code>map</code> parent frame ID.</li></ul>

### Coordinate conversion
The bridge converts between the different coordinate system conventions used by ROS (NWU, North-West-Up: right-handed, X-forwards, Y-left, Z-up) and Project AirSim (NED, North-East-Down: right-handed, X-forwards, Y-right, Z-down).

For instance, the `actual_pose` ROS topic reports the vehicle position and orientation using NWU coordinates even though the vehicle position obtained through the Project AirSim Client API are in NED coordinates.  Similarly, when setting the pose of the vehicle through the `desired_pose` ROS topic, specify the pose using NWU coordinates and the bridge converts them to NED coordinates before passing them to Project AirSim.

## Multiple Project AirSim instances
Multiple Project AirSim instances can coexist on the same ROS network, but the set of ROS topics and transform frame ID's advertised by each instance must be unique.

Each ROS topic is prefixed by <code>/Sim/*&lt;scene_name&gt;*</code> where <code>*&lt;scene_name&gt;*</code> is the string given by the `id` property in the [scene configuration file](../config_scene.md), with transform frame ID's being similar.  Therefore, load each Project AirSim instance with a scene configuration file containing a unique ID (even if the contents are otherwise the same.)
