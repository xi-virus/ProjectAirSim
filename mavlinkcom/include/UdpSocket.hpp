...
// This class represents a simple single-threaded Socket interface specific for
// Udp connections Basic socket functions are exposed through this
class UdpSocket {
 public:
  UdpSocket();
  ...
  // Used when the socket is in a connected state (so that the intended
  // recipient is known)
  int send(const void* pkt, size_t size);

  // Send message to the specified address, port
  int sendto(const void* buf, size_t size, const std::string& address,
             uint16_t port);
