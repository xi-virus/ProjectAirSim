...
#ifdef _WIN32
typedef HRESULT(WINAPI* SetThreadDescriptionFunction)(
    _In_ HANDLE hThread, _In_ PCWSTR lpThreadDescription);
static SetThreadDescriptionFunction setThreadDescriptionFunction = nullptr;
#endif

// setThreadName is a helper function that is useful when debugging because your
// threads show up in the debugger with the name you set which makes it easier
// to find the threads that you are interested in.
bool CurrentThread::setThreadName(const std::string& name) {
#ifdef _WIN32
  // unfortunately this is only available on Windows 10, and AirSim is not
  // limited to that.
  ...
  if (setThreadDescriptionFunction != nullptr) {
    ...
    auto rc =
        (*setThreadDescriptionFunction)(GetCurrentThread(), wide_path.c_str());
    return S_OK == rc;
  }
  return false;
#elif defined(__APPLE__)
