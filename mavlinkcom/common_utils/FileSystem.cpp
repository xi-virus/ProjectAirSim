...
// File names are unicode (std::wstring), because users can create folders
// containing unicode characters on both Windows, OSX and Linux.
  ...
  if (!DeleteFile(wide_path.c_str())) {
    ...
    if (hr != ERROR_FILE_NOT_FOUND) {
      throw std::runtime_error(Utils::stringf(
          "Failed to delete file '%s', error=%d.", fileName.c_str(), hr));
    }
  }
#else
...
std::string FileSystem::getUserDocumentsFolder() {
  ...
  if (0 == SHGetFolderPath(NULL, CSIDL_MYDOCUMENTS | CSIDL_FLAG_CREATE, NULL, 0,
                           szPath)) {
    std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t> converter;
...
std::string FileSystem::getFullPath(const std::string fileName) {
  ...
  if (len == 0) {
    int hr = GetLastError();
    throw std::runtime_error(Utils::stringf(
        "getFullPath of '%s' failed with error=%d.", fileName.c_str(), hr));
  }
