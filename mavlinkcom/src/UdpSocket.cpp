...
void UdpSocket::close() { pImpl->close(); }

int UdpSocket::send(const void* pkt, size_t size) {
  return pImpl->send(pkt, size);
}

int UdpSocket::sendto(const void* buf, size_t size, const std::string& address,
                      uint16_t port) {
...
bool UdpSocket::reuseaddress() { return pImpl->reuseaddress(); }

void UdpSocket::set_broadcast(void) { pImpl->set_broadcast(); }
