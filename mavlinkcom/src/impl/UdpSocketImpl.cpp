...
UdpSocketImpl::UdpSocketImpl() { fd = socket(AF_INET, SOCK_DGRAM, 0); }
...
UdpSocketImpl::~UdpSocketImpl() { close(); }

void UdpSocketImpl::make_sockaddr(const std::string& address, uint16_t port,
...
int UdpSocketImpl::connect(const std::string& address, uint16_t port) {
  ...
  int rc = ::connect(fd, reinterpret_cast<struct sockaddr*>(&sockaddr),
                     sizeof(sockaddr));
  if (rc != 0) {
...
int UdpSocketImpl::bind(const std::string& address, uint16_t port) {
  ...
  int rc = ::bind(fd, reinterpret_cast<struct sockaddr*>(&sockaddr),
                  sizeof(sockaddr));
  if (rc != 0) {
    int hr = WSAGetLastError();
    throw std::runtime_error(
        Utils::stringf("UdpSocket bind failed with error: %d\n", hr));
    return hr;
  }
  return 0;
}
...
bool UdpSocketImpl::reuseaddress(void) {
  ...
  return (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR,
                     reinterpret_cast<char*>(&one), sizeof(one)) != -1);
}
...
int UdpSocketImpl::send(const void* buf, size_t size) {
  int hr =
      ::send(fd, reinterpret_cast<const char*>(buf), static_cast<int>(size), 0);
  if (hr == SOCKET_ERROR) {
    hr = WSAGetLastError();
    throw std::runtime_error(
...
/*
  send some data
 */
int UdpSocketImpl::sendto(const void* buf, size_t size,
                          const std::string& address, uint16_t port) {
  struct sockaddr_in sockaddr;
  make_sockaddr(address, port, sockaddr);
  int hr = ::sendto(
      fd, reinterpret_cast<const char*>(buf), static_cast<int>(size), 0,
...
int UdpSocketImpl::recv(void* buf, size_t size, uint32_t timeout_ms) {
  ...
  int rc = ::recvfrom(fd, reinterpret_cast<char*>(buf), static_cast<int>(size),
                      0, reinterpret_cast<sockaddr*>(&in_addr), &len);
  if (rc < 0) {
    rc = WSAGetLastError();
...
void UdpSocketImpl::set_broadcast(void) {
  int one = 1;
  setsockopt(fd, SOL_SOCKET, SO_BROADCAST, reinterpret_cast<char*>(&one),
             sizeof(one));
}
