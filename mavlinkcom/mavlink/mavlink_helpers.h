 ...
 */
MAVLINK_HELPER uint8_t mavlink_sign_packet(
    mavlink_signing_t* signing, uint8_t signature[MAVLINK_SIGNATURE_BLOCK_LEN],
 ...
 */
MAVLINK_HELPER bool mavlink_signature_check(
    mavlink_signing_t* signing, mavlink_signing_streams_t* signing_streams,
 ...
 */
MAVLINK_HELPER uint16_t mavlink_finalize_message_buffer(
  ...
  msg->len =
      mavlink1 ? min_length : _mav_trim_payload(_MAV_PAYLOAD(msg), length);
  msg->sysid = system_id;
...
#ifndef MAVLINK_NO_SIGN_PACKET
  if (signing) {
    mavlink_sign_packet(status->signing, msg->signature, (const uint8_t*)buf,
                        header_len, (const uint8_t*)_MAV_PAYLOAD(msg), msg->len,
...
MAVLINK_HELPER uint16_t mavlink_finalize_message_chan(
  ...
  mavlink_status_t* status = mavlink_get_channel_status(chan);
  return mavlink_finalize_message_buffer(msg, system_id, component_id, status,
...
MAVLINK_HELPER void _mav_finalize_message_chan_send(
    mavlink_channel_t chan, uint32_t msgid, const char* packet,
    uint8_t min_length, uint8_t length, uint8_t crc_extra) {
  uint16_t checksum;
...
MAVLINK_HELPER void _mavlink_resend_uart(mavlink_channel_t chan,
  ...
  if (msg->magic == MAVLINK_STX_MAVLINK1) {
    ...
    // we can't send the structure directly as it has extra mavlink2 elements in
    // it
    ...
    signature_len = (msg->incompat_flags & MAVLINK_IFLAG_SIGNED)
  ...
  _mavlink_send_uart(chan, (const char*)ck, 2);
  if (signature_len != 0) {
    _mavlink_send_uart(chan, (const char*)msg->signature,
                       MAVLINK_SIGNATURE_BLOCK_LEN);
  }
...
mavlink_msg_to_send_buffer(uint8_t* buf, const mavlink_message_t* msg) {
...
MAVLINK_HELPER const mavlink_msg_entry_t* mavlink_get_msg_entry(
    uint32_t msgid) {
...
MAVLINK_HELPER uint8_t
mavlink_min_message_length(const mavlink_message_t* msg) {
  const mavlink_msg_entry_t* e = mavlink_get_msg_entry(msg->msgid);
...
MAVLINK_HELPER uint8_t
mavlink_max_message_length(const mavlink_message_t* msg) {
  const mavlink_msg_entry_t* e = mavlink_get_msg_entry(msg->msgid);
...
/* Support shorter buffers than the
   default maximum packet size */
      ...
      if (rxmsg->incompat_flags & MAVLINK_IFLAG_SIGNED) {
        ...
        if (status->signing &&
            (status->signing->accept_unsigned_callback == NULL ||
             !status->signing->accept_unsigned_callback(status,
                                                        rxmsg->msgid))) {
          // If the CRC is already wrong, don't overwrite msg_received.
          if (status->msg_received != MAVLINK_FRAMING_BAD_CRC) {
    ...
    case MAVLINK_PARSE_STATE_SIGNATURE_WAIT:
      rxmsg->signature[MAVLINK_SIGNATURE_BLOCK_LEN - status->signature_wait] =
          c;
