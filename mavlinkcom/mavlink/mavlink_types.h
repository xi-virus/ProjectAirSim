...
#define MAVLINK_CORE_HEADER_LEN \
  9  ///< Length of core header (of the comm. layer)
...
#define MAVLINK_NUM_HEADER_BYTES \
  (MAVLINK_CORE_HEADER_LEN +     \
   1)  ///< Length of all header bytes, including core and stx
...
#define MAVLINK_MAX_PACKET_LEN                               \
  (MAVLINK_MAX_PAYLOAD_LEN + MAVLINK_NUM_NON_PAYLOAD_BYTES + \
   MAVLINK_SIGNATURE_BLOCK_LEN)  ///< Maximum packet length
 ...
 * mavlink_param_union_double_t will be 8 bytes long, and treated as needing 8
 * byte alignment for the purposes of MAVLink 1.0 field ordering. The
 * mavlink_param_union_double_t will be treated as a little-endian structure.
 *
 * If is_double is 1 then the type is a double, and the remaining 63 bits are
 * the double, with the lowest bit of the mantissa zero. The intention is that
 * by replacing the is_double bit with 0 the type can be directly used as a
 * double (as the is_double bit corresponds to the lowest mantissa bit of a
 * double). If is_double is 0 then mavlink_type gives the type in the union. The
 * mavlink_types.h header will also need to have shifts/masks to define the bit
 * boundaries in the above, as bitfield ordering isn't consistent between
 * platforms. The above is intended to be for gcc on x86, which should be the
 * same as gcc on little-endian arm. When using shifts/masks the value will be
 * treated as a 64 bit unsigned number, and the bits pulled out using the
 * shifts/masks.
 */
...
MAVPACKED(typedef struct __mavlink_system {
  uint8_t
      sysid;  ///< Used by the MAVLink message_xx_send() convenience function
  uint8_t
      compid;  ///< Used by the MAVLink message_xx_send() convenience function
})
mavlink_system_t;
...
#define mavlink_ck_b(msg) \
  *(((msg)->len + (uint16_t)1) + (uint8_t*)_MAV_PAYLOAD_NON_CONST(msg))
...
#define MAVLINK_STATUS_FLAG_IN_SIGNED \
  4  // last incoming packet was signed and validated
#define MAVLINK_STATUS_FLAG_IN_BADSIG \
  8  // last incoming packet had a bad signature
...
typedef struct __mavlink_status {
  ...
  struct __mavlink_signing_streams*
      signing_streams;  ///< global record of stream timestamps
} mavlink_status_t;

/*
  a callback function to allow for accepting unsigned packets
 */
typedef bool (*mavlink_accept_unsigned_t)(const mavlink_status_t* status,
                                          uint32_t msgid);
...
/*
  timestamp state of each logical signing stream. This needs to be the same
  structure for all connections in order to be secure
 */
...
typedef struct __mavlink_signing_streams {
  ...
  struct __mavlink_signing_stream {
    ...
    uint8_t timestamp_bytes[6];  ///< Timestamp, in microseconds since UNIX
                                 ///< epoch GMT
...
typedef struct __mavlink_msg_entry {
  ...
  uint8_t max_msg_len;        // maximum message length (e.g. including mavlink2
                              // extensions)
