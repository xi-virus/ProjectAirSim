...
*/
MAVLINK_HELPER const mavlink_message_info_t* mavlink_get_message_info_by_id(
    uint32_t msgid) {
  static const mavlink_message_info_t mavlink_message_info[] =
  ...
  */
  const uint32_t count =
      sizeof(mavlink_message_info) / sizeof(mavlink_message_info[0]);
  if (count == 0) {
    return NULL;
  }
...
/*
  return the message_info struct for a message
*/
MAVLINK_HELPER const mavlink_message_info_t* mavlink_get_message_info(
...
/*
  return the message_info struct for a message
*/
MAVLINK_HELPER const mavlink_message_info_t* mavlink_get_message_info_by_name(
    const char* name) {
  static const struct {
    const char* name;
    uint32_t msgid;
  ...
  */
  const uint32_t count =
      sizeof(mavlink_message_names) / sizeof(mavlink_message_names[0]);
  if (count == 0) {
    return NULL;
  }
  uint32_t low = 0, high = count - 1;
