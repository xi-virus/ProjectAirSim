 ...
 * available online.
 *
 * Their intent is to lower the barrier for MAVLink adopters to use gimbal-lock
 * free (both rotation matrices, sometimes called DCM, and quaternions are
 * gimbal-lock free) rotation representations. Euler angles (roll, pitch, yaw)
 * will be phased out of the protocol as widely as possible.
 *
 ...
 *
 * @param quaternion a [w, x, y, z] ordered quaternion (null-rotation being 1 0
 * 0 0)
 ...
 */
MAVLINK_HELPER void mavlink_dcm_to_euler(const float dcm[3][3], float* roll,
 ...
 * 0 0)
 */
MAVLINK_HELPER void mavlink_euler_to_quaternion(float roll, float pitch,
                                                float yaw,
                                                float quaternion[4]) {
...
MAVLINK_HELPER void mavlink_dcm_to_quaternion(const float dcm[3][3],
  ...
  if (tr > 0.0f) {
    ...
    float s = sqrtf(
        (dcm[dcm_i][dcm_i] - dcm[dcm_j][dcm_j] - dcm[dcm_k][dcm_k]) + 1.0f);
